...
func startUnboundContainer(ctx context.Context, t *testing.T) (runningContainer testcontainers.Container, mappedPort nat.Port) {
	recordsConfig := newUnboundRecordsConfiguration(
		newUnboundRecord(testDomain, RecordTypeA.String(), primaryTestIPv4),
		newUnboundRecord(testDomain, RecordTypeA.String(), secondaryTestIPv4),
		newUnboundRecord(testDomain, RecordTypeAAAA.String(), primaryTestIPv6),
		newUnboundRecord(testDomain, RecordTypeAAAA.String(), secondaryTestIPv6),
	)
...
// newUnboundRecord creates a new unboundRecord instance with the provided domain, record type and IP.
func newUnboundRecord(domain, recordType, ip string) unboundRecord {
